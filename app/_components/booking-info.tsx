...
import { BarbershopSectionCentralizer, BarbershopSectionSpacing, BarbershopSectionTitle } from "./page";
...
const BookingInfo = ({ children, bookingId, status }: BookingProps) => {

    const {data: booking, isLoading, error} = useBooking(bookingId)
    const { mutate, isPending: isCancelingBooking } = useCancelBooking()
    const { mutate: isMutateReschedule, isPending: isScheduleBooking} = useRescheduleBooking()
    ...
    return (
        <Sheet>
            ...
            <div className="relative rounded-xl min-w-[400] min-h-[200]">
                <Image fill src="/map.png" alt="Mapa" className="rounded-xl object-cover"/>
                <div className="absolute flex items-center gap-2 font-semibold z-20 p-3 rounded-xl bg-card bottom-5 left-[10%]">
                    <div className="relative rounded-[20px] shrink-0 size-[40px] overflow-hidden">
                        <Image alt={booking.barbershop.name} src={booking.barbershop.imageUrl} fill className="rounded-[10px] object-cover"/>
                    </div>
                    {booking.barbershop.name}
                </div>
            </div>

            <p className={`${status == 'confirmed' ? 'bg-primary' : 'bg-destructive text-card-foreground'} place-self-start text-muted px-4 py-1 rounded-xl`}>
            ...
            <div className="flex gap-3 items-center justify-center p-4">
              ...
              ) : status === "inactive" && isAfter(booking.date, new Date()) ? (
                // INATIVO FUTURO → Re-agendar
                <Button
